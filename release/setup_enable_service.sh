...
echo -e " "
echo -e "\e[32m----------------------------------------------------\e[0m"
echo -e "\e[32m 为 fourier-grx 开启开机自启动 \e[0m"
echo -e "\e[32m Enable fourier-grx auto start \e[0m"

# 获取当前的 crontab 条目
current_crontab=$(crontab -l 2>/dev/null)

# 如果 @reboot $FOURIER_GRX_HOME/run.sh 不在当前的 crontab 中，则添加它
if ! echo "$current_crontab" | grep -q "@reboot $FOURIER_GRX_HOME/run.sh"; then
    (echo "$current_crontab"; echo "@reboot $FOURIER_GRX_HOME/run.sh") | crontab -
fi
...
# print message
echo "fourier-grx 已设置为开机自启动"
echo "fourier-grx has been set to auto start"
