...
class FourierGaitGenerator:
    ...
    def __init__(self,
        ...
        self.frequency = torch.ones(self.num_envs, 1, dtype=torch.float, device=self.device) * self.natural_frequency
        ...
        self.pattern_params = {
            "off": {
                ...
                "foot_height": torch.tensor([0.0, 0.0], device=self.device),
            },
            "stand": {
                ...
                "force_contact": torch.tensor([0.0, 0.0], device=self.device),
                "force_swing": torch.tensor([0.0, 0.0], device=self.device),
                "foot_height": torch.tensor([0.06, 0.06], device=self.device),
            },
            "walk": {
            ...
            "run": {
                ...
                "force_contact": torch.tensor([0.0, 0.0], device=self.device),
                "force_swing": torch.tensor([0.0, 0.0], device=self.device),
                "foot_height": torch.tensor([0.12, 0.12], device=self.device),
            },
        }
    ...
    def _cpg_hopf_oscillator(self):
        ...
        oscillator_frequency = self.frequency.repeat(1, self.num_feet)
    ...
    def get_frequency_norm(self, env_ids=None):
        ...
        return (self.frequency[env_ids] - self.natural_frequency) / self.natural_frequency
    ...
    def get_contact_ratio(self, env_ids=None):
        ...
        return self.contact_ratio[env_ids]
    ...
    def get_contact_ratio_norm(self, env_ids=None):
        ...
        return (self.contact_ratio[env_ids] - 0.5) / 0.5
    ...
    def set_pattern(
        ...
        if pattern in ["off", "stand", "walk", "run"]:
            ...
            self.set_foot_height(env_ids=env_ids, foot_height=self.pattern_params[pattern]["foot_height"])
        else:
...
if __name__ == "__main__":
    ...
    for i in time:
        ...
        if i == len(time) // 9 * 5:
            gait_generator.set_pattern(
                ...
                contact_ratio=torch.tensor([[0.333, 0.333]], dtype=torch.float, device="cpu"),
            )
    ...
    # subplots
    fig, axs = plt.subplots(7, 3, figsize=(12, 6))
    ...
    axs[2, 0].legend()
    axs[3, 0].plot(time, foot_height_list, label='height')
    axs[3, 0].legend()
    axs[4, 0].plot(time, contact_list, label='contact')
    axs[4, 0].legend()
    axs[5, 0].plot(time, phase_diff_list, label='phase_diff_list')
    axs[5, 0].legend()
    axs[6, 0].plot(time, swing_middle_list, label='swing_middle')
    axs[6, 0].legend()
    ...
    axs[2, 1].legend()
    axs[3, 1].plot(time, mirror_foot_height_list, label='mirror_height')
    axs[3, 1].legend()
    axs[4, 1].plot(time, mirror_contact_list, label='mirror_contact')
    axs[4, 1].legend()
    axs[5, 1].plot(time, mirror_phase_diff_list, label='mirror_phase_diff_list')
    axs[5, 1].legend()
    axs[6, 1].plot(time, mirror_swing_middle_list, label='mirror_swing_middle')
    axs[6, 1].legend()
    ...
    axs[2, 2].legend()
    axs[3, 2].plot(time, output_foot_height_list, label='output_height')
    axs[3, 2].legend()
    axs[4, 2].plot(time, output_foot_contact_list, label='output_contact')
    axs[4, 2].legend()
    axs[5, 2].plot(time, output_phase_diff_list, label='output_phase_diff_list')
    axs[5, 2].legend()
    axs[6, 2].plot(time, output_swing_middle_list, label='output_swing_middle')
    axs[6, 2].legend()
