...
class FourierGaitGenerator:
    ...
    def __init__(self,
        ...
        self.phase_diff_threshold_for_stand_still = 1 / 4
        self.phase_diff = 0.0
        ...
        self.output_selector = torch.zeros(self.num_envs, 1, dtype=torch.long, device=self.device)
        ...
        self.encoder_off = torch.tensor([0.0, 0.0, 0.0, ], device=self.device)
        self.encoder_stand = torch.tensor([1.0, 0.0, 0.0, ], device=self.device)
        self.encoder_walk = torch.tensor([0.0, 1.0, 0.0, ], device=self.device)
        self.encoder_run = torch.tensor([0.0, 0.0, 1.0, ], device=self.device)
    ...
    def set_walk_pattern(self, env_ids=None):
        ...
        self.from_stand_to_move(env_ids=env_ids, phase_offset=self.phase_offset_walk)
    ...
    def set_run_pattern(self, env_ids=None):
        ...
        self.from_stand_to_move(env_ids=env_ids, phase_offset=self.phase_offset_run)
        self.states[env_ids] = self.STATE_RUN

    def from_stand_to_move(self,
                           env_ids=None,
                           phase_offset=torch.Tensor([0.0, 0.0])):
        """
        如果是从静止到运动，那么需要立即修改 x,y 到合适的值，否则由于站立的 phase_offset 给的是 0，会导致多条腿同时抬起

        Input Params:
        - env_ids: [num_envs]
        - phase_offset: [2], range: [0, 2 * pi]
        - start_side: "left" or "right" or "random" (not "left" or "right")
        """
        if env_ids is None:
            env_ids = torch.arange(self.num_envs, device=self.device)
        else:
        ...
        """
        env_ids_of_stand_state = torch.where(self.states[env_ids] == self.STATE_STAND)[0]
        ...
        """
        Jason 2024-12-25:
        随机设置 output_selector, 可以防止机器人启动的时候只使用一侧的腿启动踏步，容易形成非 mirror 的 policy
        """
        self.set_output_selector(env_ids=env_ids_of_stand_state,
                                 output_selector=torch.randint(low=0,
                                                               high=2,
    ...
    def get_stand_pattern_env_ids(self, env_ids=None):
        ...
        env_ids = torch.where(self.states[env_ids] == self.STATE_STAND)[0]
    ...
    def get_walk_pattern_env_ids(self, env_ids=None):
        ...
        env_ids = torch.where(self.states[env_ids] == self.STATE_WALK)[0]
    ...
    def get_run_pattern_env_ids(self, env_ids=None):
        ...
        env_ids = torch.where(self.states[env_ids] == self.STATE_RUN)[0]

        return env_ids

    def get_stand_still_env_ids(self, env_ids=None):
        if env_ids is None:
            env_ids = torch.arange(self.num_envs, device=self.device)
        else:
            env_ids = env_ids
        ...
        condition1 = temp_phase_diff < self.phase_diff_threshold_for_stand_still

        # calculate the condition for phase difference being greater than (1 - threshold)
        condition2 = temp_phase_diff > (1 - self.phase_diff_threshold_for_stand_still)
    ...
    def get_foot_contact(self, env_ids=None) -> torch.Tensor:
        ...
        # situation 2: when in stand state, need phase_diff_raw < phase_diff_threshold_for_stand_still
        env_ids_of_stand_still = self.get_stand_still_env_ids()
        flags_of_foot_contact_s2 = torch.zeros(self.num_envs, self.num_feet, device=self.device)
        flags_of_foot_contact_s2[env_ids_of_stand_still] = 1
    ...
    def set_output_selector(self, env_ids=None, output_selector=None):
        if env_ids is None:
            env_ids = torch.arange(self.num_envs, device=self.device)
        else:
            env_ids = env_ids
        ...
        if output_selector is None:
            output_selector = torch.zeros(env_ids.shape[0], 1, device=self.device)
        else:
            output_selector = output_selector

        self.output_selector[env_ids] = output_selector
    ...
    def get_output_x(self, env_ids=None):
        ...
        output_x = torch.where(self.output_selector == 0, self.x, self.mirror_x)
    ...
    def get_output_y(self, env_ids=None):
        ...
        output_y = torch.where(self.output_selector == 0, self.y, self.mirror_y)
    ...
    def get_output_x_norm(self, env_ids=None):
        ...
        output_x_norm = torch.where(self.output_selector == 0, self.get_x_norm(), self.get_mirror_x_norm())
    ...
    def get_output_y_norm(self, env_ids=None):
        ...
        output_y_norm = torch.where(self.output_selector == 0, self.get_y_norm(), self.get_mirror_y_norm())
    ...
    def get_output_encoder(self, env_ids=None):
        ...
        output_encoder = torch.where(self.output_selector == 0, self.get_encoder(), self.get_mirror_encoder())
    ...
    def get_output_phase_norm(self, env_ids=None):
        ...
        output_phase_norm = torch.where(self.output_selector == 0, self.get_phase_norm(), self.get_mirror_phase_norm())
    ...
    def get_output_foot_height(self, env_ids=None, type=FOOT_HEIGHT_TYPE_SMOOTH) -> torch.Tensor:
        ...
        output_foot_height = torch.where(self.output_selector == 0,
    ...
    def get_output_foot_height_max(self, env_ids=None):
        ...
        output_foot_height_max = torch.where(self.output_selector == 0,
    ...
    def get_output_foot_height_norm(self, env_ids=None, type=FOOT_HEIGHT_TYPE_SMOOTH):
        ...
        output_foot_height_norm = torch.where(self.output_selector == 0,
    ...
    def get_output_foot_contact(self, env_ids=None):
        ...
        output_foot_contact = torch.where(self.output_selector == 0,
    ...
    def get_output_phase_diff(self, env_ids=None):
        ...
        output_phase_diff = torch.where(self.output_selector == 0,
                                        self.get_phase_diff(),
    ...
    def get_output_swing_middle(self, env_ids=None, type=SWING_MIDDLE_TYPE_LINEAR) -> torch.Tensor:
        ...
        output_swing_middle = torch.where(self.output_selector == 0,
...
if __name__ == "__main__":
    ...
    for i in time:
        ...
        phase_diff = torch.concat([phase_diff[0],
                                   torch.tensor([gait_generator.phase_diff_threshold_for_stand_still]),
                                   torch.tensor([1 - gait_generator.phase_diff_threshold_for_stand_still])])
