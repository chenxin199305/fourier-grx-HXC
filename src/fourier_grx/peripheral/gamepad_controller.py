...
from .fi_inputs import InputEvent, get_gamepad, devices, UnpluggedError
...
class GamePadController(threading.Thread):
    ...
    def unplugged(self):
        with self._lock:
            """
            Jason 2024-12-27:
            这里的 unplugged() 方法用于处理手柄被拔掉的情况，
            主要是为了在手柄被拔掉或者手柄接收器松动时，能够重置所有的状态。
            """
            # reset all states
            self.reset_state()
        ...
        while self.running:
            ...
            try:
                ...
                if self.connected:
                    ...
                    """
                    Jason 2025-07-23:
                    这里只打印一次，防止在手柄被拔掉时，出现大量的 UnpluggedError 异常。
                    """
                ...
                if device_path:
                    Logger().print_info(f"Gamepad PermissionError: {e}")
                    Logger().print_info(f"Trying to change permission of {device_path}")

                    # FIXME:
                    #  Jason 2025-07-23:
                    #  这里发现即使是没有权限访问游戏手柄设备，也可以完成授权过程，所以暂时注释掉。
                ...
                # Situations:
                # - gamepad device not found: if the gamepad device is not found when starting the gamepad controller
                Logger().print_warning(f"Gamepad FileNotFoundError: {e}")
                ...
                #
                # - gamepad unlugged:
                #   Gamepd OSError: [Errno 19] No such device
