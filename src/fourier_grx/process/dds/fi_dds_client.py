...
class DDSClient:
    def __new__(cls, *args, **kwargs):
        if not hasattr(cls, "_instance"):
            cls._instance = super(DDSClient, cls).__new__(cls)
    ...
    def publish(self, key=None, value=None) -> FunctionResult:
        ...
        if value is not None:
            data = DDSData()
            data.key(f"{self.prefix}/dynalink_interface/{key}/client")
        ...
        for key in self.keys:
            ...
            data_dict = convert_numpy(dynalink_content.write_to_dict())
    ...
    def _process_sample(self, sample: DDSData):
        ...
        # 找到对应的接口
        key_map = {
            f"{self.prefix}/dynalink_interface/{key}/server":
