...
from fi_sync_server_socket import SyncServerSocket
from fi_sync_client_socket import SyncClientSocket


def server_test(period: float = 60.0):
    server = SyncServerSocket(verbose=True)
...
def client_test(period: float = 60.0):
    client = SyncClientSocket(verbose=True)
    ...
    print(f"[Client] Discovered server at {client.server_host}:{client.server_port}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test SyncServerSocket / SyncClientSocket")
