...
from fi_sync_server_zmq import SyncServerZMQ
from fi_sync_client_zmq import SyncClientZMQ


def server_test(period: float = 60.0):
    server = SyncServerZMQ(verbose=True)
...
def client_test(period: float = 60.0):
    client = SyncClientZMQ(verbose=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test SyncServerZMQ / SyncClientZMQ")
