...
from fourier_grx.algorithm.robot_real.fi_algorithm_robot_real_clear_fault import (
    AlgorithmRobotRealClearFaultControlModel,
)
...
class TaskRobotRealClearFault(TaskRobotRealBase):
    def register(
            self,
        ...
        self.task_model = AlgorithmRobotRealClearFaultControlModel()
        self.task_key = TaskMenuRobotReal.TASK_CLEAR_FAULT
    ...
    def _function_meta(self, **kwargs) -> FunctionResult:
        ...
        if self.task_model.stage == AlgorithmStage.STAGE_INIT:
            ...
            for actuator in robot.actuators:
                actuator.set_target_control_mode(ActuatorControlMode.CLEAR_FAULT)
            ...
            robot.work_space = RobotWorkSpace.ACTUATOR_SPACE
            for actuator in robot.actuators:
                actuator.set_target_control_mode(ActuatorControlMode.EFFORT)
                actuator.set_target_position(0)
                actuator.set_target_velocity(0)
                actuator.set_target_effort(0)
                actuator.set_target_current(0)
