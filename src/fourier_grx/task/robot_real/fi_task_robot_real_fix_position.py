...
from fourier_grx.algorithm.robot_real.fi_algorithm_robot_real_fix_position import (
    AlgorithmRobotRealFixPositionControlModel,
)
...
class TaskRobotRealFixPosition(TaskRobotRealBase):
    def register(
            self,
        ...
        self.task_model = AlgorithmRobotRealFixPositionControlModel()
        self.task_key = TaskMenuRobotReal.TASK_FIX_POSITION
    ...
    def _function_meta(self, **kwargs) -> FunctionResult:
        ...
        if self.task_model.stage == AlgorithmStage.STAGE_INIT:
            ...
            for actuator in robot.actuators:
                actuator.set_target_control_mode(ActuatorControlMode.POSITION)
                actuator.set_target_position(actuator.measured_position)
