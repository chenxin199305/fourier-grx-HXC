...
from fourier_grx.algorithm.robot_real.fi_algorithm_robot_real_servo_reboot import (
    AlgorithmRobotRealServoRebootControlModel,
)
...
class TaskRobotRealServoReboot(TaskRobotRealBase):
    def register(
            self,
        ...
        self.task_model = AlgorithmRobotRealServoRebootControlModel()
        self.task_key = TaskMenuRobotReal.TASK_SERVO_REBOOT
    ...
    def _function_meta(self, **kwargs) -> FunctionResult:
        ...
        if self.task_model.stage == AlgorithmStage.STAGE_INIT:
            ...
            for actuator in robot.actuators:
                actuator.set_target_control_mode(ActuatorControlMode.SERVO_REBOOT)
            ...
            # clear buffered PID values
            for joint in robot.joints:
                joint.set_target_position_control_kp(0)
                joint.set_target_velocity_control_kp(0)
                joint.set_target_velocity_control_ki(0)
                joint.set_target_effort_control_kp(0)
                joint.set_target_effort_control_ki(0)
                joint.set_target_pd_control_kp(0)
                joint.set_target_pd_control_kd(0)
